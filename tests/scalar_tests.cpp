...
const test integer_limits("scalar: integer limits of every width", [] {
    ...
    check_equal(reflgen::json::to_string(std::uint64_t{18446744073709551615ull}), std::string("18446744073709551615"));
});
...
const test integer_narrowing("scalar: narrowing reads are range checked", [] {
    ...
    check_throws([] { reflgen::binary::from_bytes<std::uint8_t>(reflgen::binary::to_bytes(1000)); }, "out of range");
});
