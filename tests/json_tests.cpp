...
const test indented_output("json: indented output", [] {
    ...
    check_equal(reflgen::json::to_string(value, 2), std::string("{\n  \"e\": [],\n  \"k\": [\n    1,\n    2\n  ]\n}"));
});
...
const test bytes("json: bytes are base64", [] {
    ...
    check_throws([] { reflgen::json::from_string<std::vector<std::byte>>("\"ab!=\""); }, "invalid base64 character");
    check_throws([] { reflgen::json::from_string<std::vector<std::byte>>("\"a=bc\""); }, "invalid base64 character");
});
