...
namespace
{
    ...
    const test nested_round_trip("object: nested objects, containers and inheritance",
    ...
    const test external_description("object: externally described types", [] {
        ...
        check_equal(reflgen::json::to_string(objects::vendor_config{}),
                    std::string(R"({"port":80,"host":"localhost"})"));
    });
