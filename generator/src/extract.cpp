...
{
    return attribute.scope == library_scope && (attribute.name == "reflect" || attribute.name == "ignore");
}
...
class extractor
{
  ...
  private:
    ...
    std::vector<attribute_group> field_groups(CXCursor field)
    {
        ...
        std::vector<attribute_group> groups =
            groups_run_at(source.tokens, source.groups, offset_of(clang_getRangeStart(clang_getCursorExtent(field))));
        ...
        if (!context.first_hidden_member.empty() && !befriends_access(cursor))
        {
            report_.error("RG0002", position,
                          "'" + result.qualified_name + "' reflects the non-public member '" +
                              context.first_hidden_member + "'; add `friend struct reflgen::access;` to the class");
        }
    ...
    static std::map<std::string, int> count_functions(CXCursor cursor)
    {
        ...
        visit_children(cursor, [&](CXCursor child, CXCursor) {
            ...
            if (kind == CXCursor_CXXMethod || kind == CXCursor_ConversionFunction || kind == CXCursor_FunctionTemplate)
            {
    ...
    void visit_members(CXCursor cursor, member_context& context)
    {
        visit_children(cursor, [&](CXCursor child, CXCursor) {
            ...
            case CXCursor_CXXBaseSpecifier:
                ...
                {
                    context.result.bases.push_back(strip_tag(
                        take_string(clang_getTypeSpelling(clang_getCanonicalType(clang_getCursorType(child))))));
                }
                break;
            ...
            case CXCursor_FunctionTemplate:
                warn_if_reflected(child, "RG0005",
                                  "member function template '" + cursor_spelling(child) + "' cannot be reflected yet");
                break;
    ...
    void add_method(CXCursor method, member_context& context)
    {
        ...
        {
            report_.warning("RG0004", position, "static member function '" + name + "' cannot be reflected; skipped");
            return;
        }
        ...
        {
            model.parameters.push_back(cursor_spelling(clang_Cursor_getArgument(method, static_cast<unsigned>(i))));
        }
...
{
    std::vector<std::string> arguments = {"-x", "c++", "-Wno-unknown-attributes", "-I" + stub_directory};
