...
//           [--depfile FILE] HEADER... [-- CLANG_ARG...]
//
// 빌드 시스템(cmake/reflgen.cmake 의 reflgen_generate)이 부르는 것이 기본이다.
...
constexpr std::string_view usage = "usage: reflgen --module NAME --output DIR [--attribute-scope NS]... "
                                   "[--clang-args-file FILE] [--depfile FILE] HEADER... [-- CLANG_ARG...]\n";

struct cli_options
{
    extract_options extraction;
    std::string depfile; // 비면 쓰지 않는다
};

// 한 줄에 인자 하나. 빈 줄은 건너뛴다(빌드 시스템이 만든 파일의 끝 줄바꿈 등).
...
std::optional<cli_options> parse_options(int argc, char** argv, diagnostics& report)
{
    cli_options cli;
...
{
    diagnostics report;
    const std::optional<cli_options> cli = parse_options(argc, argv, report);
    if (!cli)
    {
        report.print(stderr);
