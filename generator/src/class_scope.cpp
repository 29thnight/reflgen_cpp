        ...
        void collect_member_names(CXCursor record, std::set<std::string, std::less<>>& names, int depth)
        {
            ...
            visit_children(record, [&](CXCursor child, CXCursor) {
                ...
                const bool named_member = kind == CXCursor_FieldDecl || kind == CXCursor_VarDecl ||
                                          kind == CXCursor_CXXMethod || kind == CXCursor_FunctionTemplate ||
                                          is_record(kind) || kind == CXCursor_EnumDecl ||
                                          kind == CXCursor_ClassTemplate || kind == CXCursor_TypedefDecl ||
                                          kind == CXCursor_TypeAliasDecl || kind == CXCursor_TypeAliasTemplateDecl;
