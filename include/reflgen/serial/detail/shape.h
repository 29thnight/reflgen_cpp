...
template<class R>
concept push_back_range = requires(R& range, std::ranges::range_value_t<R> item) { range.push_back(std::move(item)); };

template<class R>
concept value_insert_range = requires(R& range, std::ranges::range_value_t<R> item) { range.insert(std::move(item)); };
...
template<class R>
concept resizable_range = std::ranges::input_range<R> && !requires(R& range) { range.clear(); } && requires(R& range) {
