...
consteval std::array<attribute_ref, sizeof...(Attrs)> make_attribute_refs(const std::tuple<Attrs...>& attributes)
{
...
constexpr field_info make_field_info() noexcept
{
    ...
    return runtime_factory::field(descriptor.name, serialized_key(descriptor), type_id_of<value_type>(),
                                  type_name_of<value_type>(), type_name_of<typename descriptor_type::owner_type>(),
                                  reflected_type_getter<value_type>(), attribute_list(runtime_field_attributes<T, I>),
                                  &field_address<T, member>, write, read);
}
...
constexpr type_descriptor make_type_descriptor() noexcept
{
    ...
    return runtime_factory::type(schema_of<T>.name, type_id_of<T>(), sizeof(T), alignof(T),
                                 std::span<const field_info>(runtime_fields<T>),
                                 std::span<const base_info>(runtime_bases<T>),
